...
export function CreateAdModal() {
    ...
    async function handleCreateAd(event: FormEvent) {
        ...
        if (!selectedGame) { console.log('no selected game'); return; }
        if (!data.yearsPlaying) { console.log('no data.yearsPlaying'); return; }
        if (!data.discord) { console.log('no data.discord'); return; }
        if (!weekDays) { console.log('no weekDays'); return; }
        if (!data.hourStart) { console.log('no data.hourStart'); return; }
        if (!data.hourEnd) { console.log('no data.hourEnd'); return; }
            ...
            <Dialog.Content className='fixed bg-[#2A2634] py-8 px-10 text-white top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 rounded-lg w-[480px] shadow-lg shadow-black/25'>
                ...
                <form onSubmit={handleCreateAd} className="mt-8 flex flex-col gap-4">
                    <div className="flex flex-col gap-2">
                        ...
                        <Select.Root defaultValue={selectedGame} onValueChange={setSelectedGame}>
                            ...
                            <Select.Portal className='relative rounded'>
                                <Select.Content className='overflow-hidden'>
                                    ...
                                    <Select.Viewport className='bg-zinc-900'>
                                        ...
                                        {games.map(game => {
                                            return (
                                                <Select.Item key={game.id} value={game.id} className='bg-zinc-900 hover:bg-zinc-800 py-3 px-4 rounded text-sm text-zinc-500'>
                    ...
                    <footer className="mt-4 flex justify-end gap-4">
                        <Dialog.Close className="bg-zinc-500 px-5 h-12 rounded-md font-semibold hover:bg-zinc-600 transition">Cancelar</Dialog.Close>
                        <button className="bg-violet-500 px-5 h-12 rounded-md font-semibold flex items-center gap-3 hover:bg-violet-600 transition"
                            type="submit">
